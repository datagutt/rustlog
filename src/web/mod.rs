...
pub async fn run(app: App, mut shutdown_rx: ShutdownRx, bot_tx: Sender<BotMessage>) {
    ...
    let app = ApiRouter::new()
        ...
        .api_route(
            "/{channel_id_type}/{channel}/all",
            get_with(handlers::get_channel_logs_all, |op| {
                op.description("Get all channel logs regardless of date")
            }),
        )
        .api_route(
            "/global/users/{user_id_type}/{user}/all",
        ...
        .api_route(
            "/search",
            get_with(handlers::search_global_logs_route, |op| {
                op.description("Search all logs globally across all channels and users")
            }),
        )
        .api_route(
        ...
        .api_route(
            "/global/channels/{channel_id_type}/{channel}/{year}/{month}",
            get_with(handlers::get_channel_logs_by_month, |op| {
                op.description("Get channel logs from the given month")
            }),
        )
        .api_route(
        ...
        .api_route(
            "/global/users/{user_id_type}/{user}/{year}/{month}",
